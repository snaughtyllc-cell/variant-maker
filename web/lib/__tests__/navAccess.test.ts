...
describe("extraTabVisible", () => {
  it("shows How to for everyone in the app, including VAs and solo members", () => {
    expect(extraTabVisible("/how-to", { role: "member", is_admin: false, experience: "agency" })).toBe(
      true,
    );
    expect(
    ...
    expect(extraTabVisible("/how-to", { role: "owner", is_admin: false, experience: "agency" })).toBe(
      true,
    );
  ...
  it("still hides unknown extras", () => {
    expect(extraTabVisible("/not-a-tab", { role: "owner", is_admin: true })).toBe(false);
  });
});
