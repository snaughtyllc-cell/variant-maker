...
**Plan:** `docs/superpowers/plans/2026-08-28-keyframe-crop-drift.md`
...
The rest of the graph (eq, grain, rebuild, warp) is unchanged.

| Axis | Rule |
|---|---|
| `crop_x_frac` / `crop_y_frac` | Start. Same bands as today. Main RNG. Same seed → **same start** as now. |
| `crop_x_frac_end` / `crop_y_frac_end` | End. **Separate RNG** (must not consume the main stream — resample / GOP / trim_end stay put). |
...
2. **Later — copy-detection Chamfer** (SSCD / DINOv2 over a frame sequence,
   min-fused; default `off` on Fast). Lab: open the 25 / 50 / 75 stills and
...
still returns the pack. Do not buy those bits with shade or snow.
...
| Module | Owns |
|---|---|
| `sampler.py` | Start `crop_*_frac`; 1080 0.35–0.65; 720 y 0.90–1.00 |
| `shot.py` | `talking_head` vs `motion`; 720 TH keep; `keeps_bottom_captions` |
| `filtergraph.py` | Static `crop=iw*K:ih*K:(iw-iw*K)*X:(ih-ih*K)*Y` |
| `quality.py` | `_QUALITY_NEUTRAL` keep 1.0, x/y 0.5 |
...
- Pin live until a pack is **Signed** in `docs/ops/look-learnings.md`
