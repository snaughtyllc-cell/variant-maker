...
Lab-only, default **off**. Hidden when `/api/health` is not `lab`. The API
rejects `prep_mode=hq` unless `VARIANT_LAB` is on. Not an HQ quality dropdown.
The Advanced HQ option stays disabled.
