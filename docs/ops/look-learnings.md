...
| Compete vignette 0.02–0.12 always-on | Jeff 2026-09-12 on the motion 3-pack: **whole clip too dark** vs source. `lookcompete4` vig **0.114** was already black-corner look-fail. Cap 0–0.04 **still dark** on new gens — ffmpeg `vignette=angle` started at PI/5, so every vig>0 was default-strength. **Skip vignette** (0) until a mapping is look-signed. Lab Fast recycle after this image. **Lab only. Live stays `c497505`.** | Raise 24. Shade. Restore 0.12/0.20. Pin live without stills. PATCH live. |
