...
| **2026-09-12** | Lab Fast vignette-off `fc5a8109…` (motion 3-pack after PI/5 cap still-dark) | Compete vig **0** (skip). Rotate + per-copy 30/48/60 stay. Medium. Lab only. | — | **Jeff: its fixed.** Dark wash was Lab-only (new Lab image + ffmpeg PI/5). **Live Fast unchanged** `c497505` — Jeff: Live gens today were fine. Do not PATCH live. |
...
| Lab Fast identity A–D+C2 on motion (`6e8dfa80fba8`, digest `80564ee0…`) | File identity landed (SHA ≠ source/peers, AAC never copy, SEI off). Copy 2 MAE **61.33** `review_required` on a 6.5s 720 motion clip — uniqueness stayed `ok`. MP4s left the keep window. **Not SaveInta. Not Jeff-signed.** | Treat A–D as a look sign. Raise 24. Redraw shade. Promote live. Redraw a Rejected row from this MAE. |
| Compete vignette 0.02–0.12 always-on | **Lab only.** Jeff 2026-09-12: Lab motion pack **whole clip too dark**. Cap 0–0.04 still dark (ffmpeg PI/5). Skip vig (0) on `fc5a8109…`. **Jeff: its fixed.** Live Fast stayed `c497505` all day — Jeff: Live gens were fine. Do not copy Lab skip onto Live. | Raise 24. Shade. Restore 0.12/0.20. PATCH live. |
