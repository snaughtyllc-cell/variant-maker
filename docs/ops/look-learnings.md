...
| **2026-08-31** | NEW pack `c701c9fb3594` (0409 / 1277 / bradnded) vs source | Signed medium + crop-align MAE + color stills. Fast 2. Lab `21691c4`. | **43/44**, **34/31**, **32/32** | **Jeff: stills look good.** Speed later. SSIM still the 24-cleared medium band (same as `6f506c681f8b`). CopyID `record` now scores audio so we can learn; do not `gate`. **Do not pin live yet.** |
...
| Lab Generate wait / 20 min NEW pack | **Parked.** Jeff 2026-08-31: stills look good; speed later. SSIM bits in the same 24-cleared medium band. CopyID `record` scores. Live pin is a separate call. | `gate`. Recycle live Fast. Raise 38. Speed work next. |
