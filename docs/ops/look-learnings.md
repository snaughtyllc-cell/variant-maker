...
| Look-safe crop pan (start→end) | **Open → handheld v1.** First pack `lookcroppan` (`7ef5853`) barely moved and had small hard shifts. Lab now `bc88da1` / `sha256:b7ab714f…`: travel floor + smoothstep + two-sine wander + 2× crop. Watch Lab Gallery `lookhand10` (10 Live Studio sources × Fast 2). | Ken Burns zoom. Face-zoom 0.72. Gemini on variants. Pin live before stills are **Signed**. |
