...
| Compete vignette 0.02–0.12 always-on | Jeff 2026-09-12 on the motion 3-pack: **whole clip too dark** vs source. `lookcompete4` vig **0.114** was already black-corner look-fail. Cap medium/strong to **0–0.04** (can skip). `:lab` digest `sha256:8067b6c1…` on GHCR (CI `34697248914` / `tier1`). Recycle **lab** `xar25v77v3j27u` by hand — GitHub `RUNPOD_API_KEY` empty. **Lab only. Live stays `c497505`.** Generate a **new** pack; `6e8dfa80fba8` still has the old vig. | Raise 24. Shade. Restore 0.12/0.20. Pin live on this cap without stills. PATCH live. |
