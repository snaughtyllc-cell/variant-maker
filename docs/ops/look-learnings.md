...
| **2026-08-31** | NEW pack `c701c9fb3594` (0409 / 1277 / bradnded) vs source | Signed medium + crop-align MAE + color stills. Fast 2. Lab `21691c4`. | **43/44**, **34/31**, **32/32** | **Jeff: stills look good.** Speed later. SSIM still the 24-cleared medium band. CopyID `record` lab only; live pin **no `VF_LAB`**, copyid **off**. Writeup: `docs/ops/live-pin-21691c4-2026-08-31.md`. |
...
| Lab Generate wait / 20 min NEW pack | **Parked.** Jeff signed stills; speed later. Live pin `21691c4` copyid **off**. | `gate`. Raise 38. Speed work next. |
...
**19 bits / ~30%** is the *post-hunt* ship floor, not a first-pass shortcut: hunt 24 (medium, then one escalate). After that, 19–23 still ship. Under 19 is `uniqueness_fail` (not Drive-ready). TikFusion’s published floor is ~18 / ~28%. Live Fast is `21691c4` (`sha256:e8d77b9f…`, color stills + crop-align MAE, **no `VF_LAB`**, copyid **off**). Prior live `e7ab2cc` (`sha256:ceb96800…`). Writeup: `docs/ops/live-pin-21691c4-2026-08-31.md`.
