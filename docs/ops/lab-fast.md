...
- Image: `ghcr.io/snaughtyllc-cell/variant-fast@sha256:b08ad7a1e3ab7049ee5f72050365d9f79eb9f31e23500ed68d024a294097f163`
- `VF_ENGINE_REV=815a262` (720 cloud 4–7 + gblur 4 + luma-only dust 14–20). `VF_LAB=1`
...
- Live SaveInta look-test (ship-loop Gallery `looktest4c41`): cloud 6–10 + sigma=2 still **chroma a bit noticeable**. Lab `568973c` 4–7 + gblur 4 (`softestd3ce5`): c1s=5, **24/24 bits (38%)**, VMAF 100/98 — look better, uniqueness too low. Lab now `815a262` / `b08ad7a1…`: same cloud + **luma-only dust 14–20**. SaveInta pack `softdust815a` (`720-cloud-softer-bits-test.mp4`): **25/26 bits (39/41%)**, VMAF **94.2 / 93.8**, c1s=5, c0s=15/17, sigma=4, medium, no escalate. Gate stays 24. Do not PATCH live to try it.
...
| `815a262` + luma dust 14–20 (`softdust815a`, SaveInta) **lab; not live** | **25/26 bits (39/41%)**, VMAF **94.2 / 93.8**, c1s=5, c0s=15/17, sigma=4, medium, no escalate. Luma-only dust moved VMAF more than SSIM bits | — |
