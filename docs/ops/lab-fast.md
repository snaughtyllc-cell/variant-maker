...
**Current lab experiment (2026-08-28): look-safe crop pan + phone Studio UI.**
Engine on Lab Fast is `7ef5853` (PR #56). Studio UI is
`cursor/mobile-studio-gallery-c975` (PR #54) — that stack was never merged,
so a `railway up` from the crop-pan/`tier1` tree wipes the phone chrome.
Generate only on [Lab Studio](https://varyforge-studio-lab.up.railway.app).
Live Studio / live Fast stay on `f05d803`. Do **not** rebuild or re-pin Fast
from the UI branch. Do not pin live until **Signed** in
...
Live Fast `j0b1q4iuunzhnq` is on `sha256:00564ea3…` / `f05d803` (19-bit / 30%
ship floor, **no `VF_LAB`**, max **4**, idle 600). Prior live `472ab60` /
`sha256:e7474975…` (look stills). Railway `RUNPOD_FAST_ENDPOINT_ID` stays the
live id. Do **not** PATCH live to test the next experiment — use lab. Do
**not** re-pin live to a new ship-loop `:latest` digest until that build is
tested. Writeup: `docs/ops/live-pin-f05d803-2026-08-26.md`.
...
| **`7ef5853` look-safe crop pan — lab only** | **Open.** Lab Fast pinned 2026-08-28. Generate on Lab Studio. Check 25/50/75 stills for window creep; captions + face stay. Do not pin live until **Signed**. Do not `railway up` Studio from this digest's `tier1` tree — that wipes the unmerged phone UI. | — |
