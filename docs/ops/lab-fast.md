...
- Image: `ghcr.io/snaughtyllc-cell/variant-fast@sha256:f6bd9b1d269e58dae48006958cbcd94f90b367c42a526ca0768a4ba76eea4a54`
- Vignette cap **0–0.04** (can skip) + BtbN dated autobuild ffmpeg. `VF_LAB=1`. `VARIANT_MAKER_COPYID=record`
- CI `34697929520` on `tier1` (`bf206f2`) pushed `:lab`. GitHub `RUNPOD_API_KEY` empty. Lab Studio boot pin hit v2 `403` (job-submit key). Boot now falls back to v1 REST + GraphQL `?api_key=` and pins the GHCR `:lab` **digest**. Optional Railway `RUNPOD_ENDPOINT_UPDATE_KEY` (write) if the job key stays 403. **Lab `xar25v77v3j27u` only. Do not PATCH live.**
...
| **`f6bd9b1d…` vignette cap 0–0.04 (`sha256:f6bd9b1d…eea4a54`) — GHCR, recycle lab** | Latest `:lab` (CI `34697929520` / `tier1`). Cap is in the image. Lab Studio `/api/health` `lab_fast_pin=error_403` on v2 PATCH — boot retries v1 + GraphQL. **Lab `xar25v77v3j27u` only. Do not PATCH live.** Generate a **new** pack; `6e8dfa80fba8` still has 0.02–0.12 vig. | — |
