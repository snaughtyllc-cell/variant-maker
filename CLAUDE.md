...
- Two xfail tests in `test_sampler.py` are the Phase-3 targets — flip them to real asserts
  as you implement `sample()`.
...
variant-maker --help            # CLI (options wired; run body lands in Phase 7)
...
| `sampler.py` | 🔨 Phase 3 | seed → budgeted, zero-mean params (pure) |
| `filtergraph.py` | 🔨 Phase 4 | params → -vf/-af in the documented order (pure) |
| `ffmpeg.py` | 🔨 Phase 5 | build + run one render; tag color on output |
| `quality.py` | 🔨 Phase 6 | histogram sanity + VMAF quality-render guard |
| `pipeline.py` | 🔨 Phase 7 | orchestrate the per-variant loop → manifest |
| `cli.py` | ◑ wired | options done; calls pipeline.run |
| `neural/*` | 🔨 Phase 8-10 | Tier 2: upscale, interpolate, protect |
